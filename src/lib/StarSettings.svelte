<script>
  let starCount = [ 0, 1, 2, 3, 4, 5 ]

  let starColors = [ 'SSS', 'SS', 'S', 'A', 'B', 'C', 'D', 'E', 'F', 'G' ]

  let starTypes = [ 'Full', 'Empty', 'Pig', 'Snek', 'Black', 'White' ]

  let starCnt = 0;
  let starColor = 'C';
  let starType = 'Full';

  let selectedValue = `/star/${starType}/${starColor}.png`;
